...
| [S1](#s1-and-what-is-actually-wrong-with-that-page) | fourteen | The universe settings page is six stacked cards and I do not like any of it. | The two-pane shape the account settings already have, with named groups and one sentence per setting (the rail half amended by Y5, Round twenty-one: the universe page drops it and keeps the named groups as plain headings; the account pages keep the rail, since theirs is real route links) |  |
...
## Round twenty-one, decided 2026-08-25

Ten findings from one pass over preview, filed as #788 to #797 under the epic #787. These were
not drawn: every one either amends a decision whose replacement Lorenzo specified himself in
the session that filed them, or fixes a surface against decisions that already exist. Rows are
added here by the wave that implements them; the prose stays short on purpose.

| Id  | Question                                                                                    | Chosen                                                                                                                                                                                                             |
| --- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Y1  | The world home is reachable only through the switcher and the entries page's back link.    | **A pinned home row in the sidebar, above the seven** (#788). A2's cap counts the seven places, not this row, the way it never counted the switcher. O1's back link on the entries page is removed rather than kept as a second convention. The nav order becomes Entries, Proposals, Table, Players, Works, Import, Settings, the order of actual use rather than the order the artifact drew |
| Y2  | The entries table loses its column headers to the scroll, and sort lives in those headers. | **The header sticks under the band** (#789). `Page` republishes the band's measured height as `--page-band-h`; the entries table leaves `TableScroll` (a scroll container would capture the sticky) for an `overflow-clip` wrapper |
| Y3  | The Ask answer panel's guardrail 5 disclosure stood as a full paragraph on every render, in both the dock and the record page, and each citation was a bordered, rounded-full pill under its quote. | **An info button opening a popover, and a plain-link citation list** (#797). `QuickAsk.svelte`'s standing paragraph and the Ask record page's own copy both become an info button (`disclosureLabel`/`noteLabel`) that opens a `Popover` carrying the same text and the same privacy link - guardrail 5 stays satisfied, the full text is one click away, it no longer takes a permanent row. Each citation drops its bordered pill for the app's own quote treatment plus a plain inline link (#551) under it, own canon or indexed said in text rather than a badge. `sourcesEmpty` drops its call-to-action sentence and keeps only the one fact guardrail 2/3 need it to carry: general knowledge, not your canon |
| Y4  | The relation catalogue lived at `/w/[universe]/settings/relations`, one level under a page nobody would guess to look, felt from nowhere a relation is actually read, and its list read like a debug table. | **A first-class route, `/w/[universe]/relations`, restyled to the product's own table conventions** (#795, amends U1's "inside the settings shell" clause). Linked from both the Settings page's Canon group and the Entries surface now, not from Settings alone; the old path 308-redirects. `RelationTypeTable` trades U1's `<ul>`/`<li>` list for a real `<table>` per section (own types, the shipped ten), with a `Uses` column replacing the old `usageCount` sentence - the rest of U1 holds: own types keep the room and every action, the shipped ten stay a compact read-only reference below with no badge repeating the heading |
| Y5  | The universe settings page's own nested rail (S1's own shape) never earned the column next to the sidebar's nav for three groups on one page, and the page scrolled roughly 650px past where its content actually ended. | **The rail goes, the three groups become plain `h2` sections carrying S1's own unset mark, and the scroll is fixed at its source** (#794, amends S1's rail half). `UniverseSettingsRail`/`SettingsShell`'s two-pane layout drop from `w/[universe]/settings/+page.svelte`; `SettingsShell` stays for the six account panes, whose rail is real route links (`SettingsNav`), not anchors. The scroll was never page-specific: `AppShell.svelte`'s `<main>` (`overflow-y-auto`) was `position: static`, so it was never a containing block for its `position: absolute` sr-only descendants (every hidden legend, radio, checkbox a form anywhere hides visually) - the same defect #652 already named for a horizontal scroller. Their CSS static position fell back to the document root, and the outer window inherited however far down the last one fell in the fully unclipped flow. `relative` on `main` fixes it for every route. Measured on this page: `documentElement.scrollHeight` 1555 to 900, an 812px scrollport holding 2422px of real, correctly-scrolling content |  |
| Y6  | Each narration style card's example sentence (issue #451, decision U2) sat small and italic, always English regardless of the reader's own interface locale. | **Body-size, non-italic text, coalesced through `narration_style_label`'s own per-locale table the same way the preset's name and description already are** (#796). `narration_style_label` gains an `example_sentence` column (migration 0056, Italian seeded for the five shipped presets); `listNarrationStylePresets` coalesces it against the shipped English row exactly the way it already coalesces name/description, falling back to English rather than showing nothing for a locale with no translation yet. The presets are DB rows with locale-keyed translations already (U2), so a DB column follows that model rather than growing a parallel i18n catalogue keyed by preset id. The universe itself carries no language of its own; the picker already threaded the signed-in reader's own interface locale into this same query for name/description, so the example sentence now reads that same signal |  |
| Y7  | `/w/[universe]/import`'s empty state was a bare "start an import" button leading to the same small file input, and the flow asked which playbook before it had ever seen the file: upload, then confirm the detected playbook, then a separate estimate screen, each its own click. | **File-first: the empty state is the drop zone itself, and confirm/estimate collapse into one preview screen behind a single "Start import" button** (#790). The drop zone (drag-and-drop plus click-to-browse, no-JS form fallback kept) posts straight to the existing `upload` action, which now runs detection and computes the estimate in the same round trip rather than waiting for a separate confirm click; overriding the detected playbook re-submits the same preview rather than opening a second screen. `packages/import` gained one narrow read, `importJobDocumentsSettled` (`estimate.ts`, alongside its existing unexported `checkpointedDocuments`), so the job review page can show a real per-document progress bar next to its proposal count; the poll that keeps it live tightened from 4s to 1.5s. `/onboarding/import`'s own upload -> confirm -> estimate steps are unchanged, out of #790's scope. |
