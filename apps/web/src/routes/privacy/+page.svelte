...
<DocPage title={t.title}>
	...
	<p>
		...
		to. So is every question you ask the Loremaster and the answer it gives, kept automatically now,
		grouped by conversation, until you delete it - a universe's own Loremaster conversations are the
		searchable record at <code>/ask</code>, with a delete control on each conversation there. It is
		a note in our database exactly like an entry, never sent anywhere but through the same gateway
		call that answered the question in the first place. Replicate and ElevenLabs, called directly
		for images and sound, sit outside this gateway control entirely - we do not have a stated
		no-training guarantee from either one yet, and this page will say so plainly the day one exists
