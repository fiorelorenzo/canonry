...
<svelte:element this={page.data.user ? 'div' : 'main'}>
	<PageHeader eyebrow="Internal component gallery, not a product page" title="AI text marking" />
	<PageBody width="working">
		<p class="mb-8 max-w-measure text-ink-2">
			Decision C1 = B: unaccepted AI wording gets a dashed underline plus a numbered margin marker,
			in the one hue reserved for the copilot, never hue alone. Proposals do not exist in the
			database yet (<a class="underline" href="https://github.com/fiorelorenzo/canonry/issues/47"
				>#47</a
			>), so this renders <code>AiMarkedParagraph</code> directly against fixture text rather than a real
			entry.
		</p>
