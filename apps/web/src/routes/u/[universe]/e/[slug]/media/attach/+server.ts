...
export const POST: RequestHandler = async ({ request, params, locals }) => {
	...
	const mediaAssetId =
		typeof body === 'object' && body !== null && 'mediaAssetId' in body
			? body.mediaAssetId
			: undefined;
	if (typeof mediaAssetId !== 'string') error(400, 'mediaAssetId must be a string');

	const existing = await mediaAssetById(context.conn, mediaAssetId);
	if (!existing || existing.universeId !== context.universe.id) {
		error(404, 'No such generated image in this universe');
	}
	...
	try {
		...
		error(409, 'That image is already attached to an entry, or does not exist.');
	}
};
