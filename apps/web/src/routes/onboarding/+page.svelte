<script lang="ts">
	 ...
	 * /u/new used to do under its own URL), or derive from a pre-indexed universe
	...
	<form method="POST" class="flex flex-col gap-6">
		...
		<div class="grid gap-4 sm:grid-cols-3">
			...
			<div class="flex flex-col gap-3 rounded-lg border border-line bg-panel p-4">
				...
				<Button type="submit" formaction="?/empty" variant="secondary" class="self-start">
					{t.emptyCard.cta}
				</Button>
			</div>
			...
			<div class="flex flex-col gap-3 rounded-lg border border-line bg-panel p-4">
				{#if data.preIndexedBase}
					...
					<Button type="submit" formaction="?/preindexed" variant="secondary" class="self-start">
