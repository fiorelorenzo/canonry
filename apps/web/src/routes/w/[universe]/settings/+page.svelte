<script lang="ts">
	/**
	 * Issue #406 (S1, DECISIONS.md "Round fourteen") built this page as the two-pane
	 * shell issue #143 (I6 = B) built for the account - a same-page-anchor rail on the
	 * left, one content pane on the right, via `SettingsShell`
	 * (`$lib/components/settings/SettingsShell.svelte`). Issue #794 (DECISIONS.md
	 * "Round twenty-one") drops the rail: three groups on one page never earned a
	 * second navigation surface next to the one the sidebar already draws, and the
	 * nested rail's own `nav` sat inside `main`'s single scrollport for no reason
	 * other than that `SettingsShell` asked for a snippet to put there. `SettingsShell`
	 * itself is untouched and still frames the six account panes at `/settings/*`
	 * (`routes/settings/+layout.svelte`) - a universe's own settings page just stops
	 * being one of its two callers. Three named groups still replace the seven
	 * ungrouped cards this page used to be, in this order: Images (the image style
	 * form, untouched - #407 lands a picker on top of it), The Loremaster (the voice,
	 * the writing switch and the propagation cap - all three about how much the
	 * copilot may do), and Canon (the relation catalogue link and precedence for a
	 * derived world). The setup checklist card stays gone: `imagesUnset`/
	 * `loremasterUnset` below read the same `universeSetupItems()` payload the shell
	 * row counts (`+page.server.ts`'s own `setupItems`) and turn it into the small
	 * mark on the owning group's own `h2` that the rail's matching row used to carry -
	 * the same signal, moved onto the heading somebody scanning the page already reads
	 * rather than a nav column that scrolled past it.
	 *
	 ...
	 * Decision K1 (DECISIONS.md "Round six"): a card here links to `/relations`,
	 * issue #192's catalogue (moved off `settings/relations` by #795, DECISIONS.md
	 * "Round twenty-one"), rather than inlining a page's worth of tables and dialogs.
	...
	// Issue #794 (DECISIONS.md "Round twenty-one"): the same per-group unset signal the
	// old rail carried on its "images"/"loremaster" rows (issue #406 S1), now read
	// straight onto the group's own `h2` instead of a nav row pointing at it. Canon
	// owns no checklist item today, so its heading never marks - same as the rail's own
	// row never did.
	const imagesUnset = $derived(
		data.setupItems.some((item) => item.id === 'imageStyle' && !item.done)
	);
	const loremasterUnset = $derived(
		data.setupItems.some((item) => item.id === 'loremasterVoice' && !item.done)
	);
</script>
...
<Page width="working" title={t.heading}>
	...
	<section id="group-images" class="mt-8">
		<h2 class="flex items-center gap-2 text-title font-semibold text-ink">
			{t.groups.images}
			{#if imagesUnset}
				<span
		...
		<div class="mt-3 flex flex-col gap-4">
			<div class="border-t border-line pt-4">
				...
				<!-- Issue #451, decision U2: the picker's own card shape, copied from the image
			     style grid above with one substitution - an example sentence in place of the
			     example image, since a voice has nothing to show but something to read aloud.
			     Issue #796 (DECISIONS.md "Round twenty-one", Y6): the sentence is body size,
			     never italic - a caption-sized quote was unreadable for a sample that is the
			     entire pitch of the card - and it reads in `data.locale`, the same signed-in
			     reader's locale the card's name/description already coalesce through
			     `listNarrationStylePresets` (queries/narration.ts), which now does the same
			     for `example_sentence` via `narration_style_label`. Picking a preset posts
			     `?/selectNarrationStylePreset`, which only ever points
			     universe.narration_style_id at the shipped row (queries/narration.ts's
			     selectUniverseNarrationStylePreset) - it never copies the preset's clause
			     anywhere, so an improved preset improves every world that chose it. -->
					...
					>
						{#each data.narrationStylePresets as preset (preset.id)}
							...
							>
								...
								/>
								<p class="text-body text-ink-2">&ldquo;{preset.exampleSentence}&rdquo;</p>
		...
		<div class="mt-3 flex flex-col gap-4">
			<div class="border-t border-line pt-4">
				<div class="flex items-center justify-between gap-4">
					...
					<Button href={resolve(`/w/${data.current.slug}/relations`)} variant="secondary">
