<script lang="ts">
	 ...
	 *
	 * Issue #121: `NAV_ITEMS.label` stays an English id-like fallback/type
	 * discriminant (nav.ts's own doc comment) - the visible label is always looked up
	 * from `t.universe.nav[item.id]` instead, so the seven nav entries are real
	 * translated strings rather than the raw data field.
	 */
...
>
	<div class="border-b border-line p-3">
		...
		>
			...
			<span>{t.sidebar.askTheLoremaster}</span>
			<span class="ml-auto font-mono text-[10px] text-muted">⌘⇧A</span>
		</a>
	</div>

	<nav class="flex-1 p-2" aria-label={t.sidebar.primaryNavAriaLabel}>
			...
			{#each NAV_ITEMS as item (item.id)}
				...
				<li>
					<a
						...
						title={item.built ? undefined : t.sidebar.notBuiltYet(item.issue)}
					>
